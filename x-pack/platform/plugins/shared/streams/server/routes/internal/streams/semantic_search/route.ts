...
import {
  ...
  semanticSearchMapping,
} from './semantic_search_params';

const META_LAYER_INDEX = 'kibana-meta-layer-streams';

const createMetaLayerIndex = async (esClient: ElasticsearchClient) => {
  ...
  await esClient.indices.create({
    index: META_LAYER_INDEX,
    mappings: semanticSearchMapping,
  });
};
...
const indexStreamsForSemanticSearchRoute = createServerRoute({
  ...
  handler: async ({
    ...
    try {
      await createMetaLayerIndex(esClient);
