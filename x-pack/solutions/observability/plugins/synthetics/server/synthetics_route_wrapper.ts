...
import { Privileges } from '@kbn/core-http-server/src/router/route';
...
export const syntheticsRouteWrapper: SyntheticsRouteWrapper = (
  ...
  const requiredPrivileges: Privileges = ['uptime-read'];
  if (syntheticsRoute.requiredPrivileges) {
    requiredPrivileges.push(...syntheticsRoute.requiredPrivileges);
  }
  if (syntheticsRoute.writeAccess) {
    if (syntheticsRoute.isAlertRoute) {
      requiredPrivileges.push({ anyRequired: ['uptime-alerts', 'uptime-write'] });
    } else {
      requiredPrivileges.push('uptime-write');
    }
  }
